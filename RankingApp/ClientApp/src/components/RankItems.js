...
const RankItems = ({ items, setItems, dataType, imgArr, localStorageKey }) => {
    ...
    function getDataFromApi() {
        ...
        const baseUrl = axios.create({ baseURL: "https://localhost:44433/itemthing/" });

        if (dataType === 1) {
            baseUrl.get("1").then((results) => { console.log("I was here"); return results.data })
                .then((data) => setItems(data));
        }
        else if (dataType === 2) {
            baseUrl.get("2").then((results) => { console.log("I was here for albums"); return results.data })
                .then((data) => setItems(data));
        }
        
        //fetch(`itemthing/${dataType}`)
        //    .then((results) => {
        //        return results.json();
        //    })
        //    .then(data => {
